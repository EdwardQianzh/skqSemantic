...
public class QuadTreeNode implements Serializable {
    ...
    public final static float DEFAULT_MIN_SIZE = 6;
    ...
    /**
     * Get k objects closest to a lat/lon. If there are children at this node,
     * then the children are searched. The children are checked first, to see if
     * they are closer than the best distance already found. If a closer object
     * is found, bestDistance will be updated with a new Double object that has
     * the new distance.
     ...
     * @param lat up-down location in QuadTree Grid (latitude, y)
     * @param lon left-right location in QuadTree Grid (longitude, x)
     * @param knn the closest distance of the object found so far.
     * @return the object that matches the best distance, null if no closer
     *         object was found.
     */
    public Collection getknn(float lat, float lon, int knn) {
    	TreeMap PQ = new TreeMap();//���ȼ�����,��ѡԪ��
    	TreeMap topk = new TreeMap();//topk
    	return  getknnbase(lat, lon,  knn, PQ,topk,new MutableDistance(Double.POSITIVE_INFINITY));
    }
     ...
     *         object was found.
     */
    public Collection getknnwithindistance(float lat, float lon, int knn, double withinDistance) {
    	Vector vector = new Vector();
    	TreeMap PQ = new TreeMap();//���ȼ�����,��ѡԪ��
    	TreeMap topk = new TreeMap();//topk
    	return getknnbase( lat,  lon,  knn, PQ,topk, new MutableDistance(withinDistance)); 
    }
    
    public Collection getknnbase(float lat, float lon, int knn,TreeMap PQ,TreeMap topk, MutableDistance bestDistance) {
    	//Vector kclosest = new Vector();
            ...
            for (QuadTreeNode child : children) {
              ...
              //  if (childDistance < bestDistance.value) {
                	//System.out.println("111111111111");
              		//System.out.println(child instanceof QuadTreeNode);
              	
              	  	PQ.put(childDistance, child);
              	  	// System.out.println(PQ.get(childDistance) instanceof QuadTreeNode);
              	  	//System.out.println("111111111111");
              //  }            
            }
       ...
       // System.out.println("PQ  :"+ (PQ.size()));
        while(true){
        	...
        	if(PQ == null){
        		...
        		//System.out.println("topk :"+topk.size());
        		if(topk.size() <knn ){
            		if(PQ.get(PQ.firstKey()) instanceof  QuadTreeLeaf){
            		//	System.out.println("111111111111"); 
            			
            		//	System.out.println("PQ"+((QuadTreeLeaf)PQ.get(PQ.firstKey())).object);
                		topk.put(PQ.firstKey(), PQ.get(PQ.firstKey()));
                		//System.out.println("topk"+topk.get(PQ.firstKey()));
                		PQ.remove(PQ.firstKey());
                	// System.out.println("PQ  :"+ (PQ.size()));
                	//	System.out.println("111111111111"); 
                	}else{
                		TreeMap PQ2 = (TreeMap) PQ.clone();
                		PQ2.remove(PQ2.firstKey());
                		QuadTreeNode temp = (QuadTreeNode) PQ.get(PQ.firstKey());
                	//	System.out.println("PQ2  :"+ (PQ2.size()));
                		temp.getknnbase( lat,lon,knn,PQ2,topk,bestDistance);
                		return  topk.values();
                	}
            	}else{
            		//System.out.println("1234567890");
            		if(Double.parseDouble(PQ.firstKey().toString()) >= Double.parseDouble(topk.lastKey().toString())){
            			System.out.println("return ");
            			return  topk.values();
            		}else{
            		//	System.out.println("QuadTreeLeaf"+(PQ.get(PQ.firstKey()) instanceof  QuadTreeLeaf));
                	//	System.out.println("QuadTreeNode"+(PQ.get(PQ.firstKey()) instanceof  QuadTreeNode));
                	//	System.out.println("PQ21343  :"+ (PQ.size()));
                		if(PQ.get(PQ.firstKey())  instanceof  QuadTreeLeaf){
                    ...
                    //	System.out.println("PQ21343  :"+ (PQ.size()));
                         	...
                         	if(PQ.get(PQ.firstKey()) instanceof  QuadTreeNode){
                             		TreeMap PQ2 = (TreeMap) PQ.clone();
                        //     		System.out.println("PQ  :"+ (PQ.size()));
                      //       		System.out.println("PQ21  :"+ (PQ2.size()));
                             		PQ2.remove(PQ2.firstKey());
                             		QuadTreeNode temp = (QuadTreeNode) PQ.get(PQ.firstKey());
                           //  		System.out.println("PQ22  :"+ (PQ2.size()));
                             		temp.getknnbase( lat,  lon, knn, PQ2,topk,bestDistance);
                             		return  topk.values();
                             }
                         }
