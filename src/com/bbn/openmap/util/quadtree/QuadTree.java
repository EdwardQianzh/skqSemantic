...
public class QuadTree implements DataOrganizer, Serializable {
    ...
    /**
      * Get k objects within a bounding box, and return the objects within
     * a given Vector.
     * 
     * @param lat up-down location in QuadTree Grid (latitude, y)
     * @param lon left-right location in QuadTree Grid (longitude, x)
     * @param knn the number of location required by the user
     *       
     * @return the object that was found, null if nothing is within the maximum
     *         distance.
     */
    public Collection getknn(float lat, float lon, int knn) {
        return top.getknn(lat, lon, knn);
    }
   ...
   public Collection getknnwithindistance(float lat, float lon, int knn, double withinDistance) {
       return top.getknnwithindistance(lat, lon, knn, withinDistance);
   }
